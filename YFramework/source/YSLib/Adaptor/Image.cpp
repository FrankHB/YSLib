...
NativePixmap::NativePixmap(const Size& s, BitPerPixel bpp)
	: bitmap(::FreeImage_Allocate(s.Width, s.Height, bpp, 0, 0, 0))
{
	if(!bitmap)
		throw BadImageAlloc();
}
NativePixmap::NativePixmap(DataPtr ptr) ynothrow
	: bitmap(ptr)
{
	YAssert(bitmap, "Null pointer found");
}
NativePixmap::NativePixmap(const string& filename)
...
NativePixmap::NativePixmap(const ImageMemory& mem)
...
NativePixmap::NativePixmap(const NativePixmap& pixmap)
	: bitmap(::FreeImage_Clone(pixmap.bitmap))
{
	if(!bitmap)
		throw BadImageAlloc();
}
NativePixmap::NativePixmap(NativePixmap&& pixmap) ynothrow
...
BitPerPixel
NativePixmap::GetBPP() const ynothrow
{
	return ::FreeImage_GetBPP(bitmap);
}
SDst
NativePixmap::GetHeight() const ynothrow
{
	return ::FreeImage_GetHeight(bitmap); 
}
SDst
NativePixmap::GetPitch() const ynothrow
{
	return ::FreeImage_GetPitch(bitmap); 
}
SDst
NativePixmap::GetWidth() const ynothrow
{
...
ImageCodec::Load(const vector<octet>& vec)
{
	...
	return Convert(NativePixmap(mem));
}

BitmapBuffer
ImageCodec::Convert(const NativePixmap& pixmap)
{
	...
	return BitmapBuffer(std::move(pixels), s);
