...
\since build 974
\par 创建时间:
	2023-05-16 01:05:47 +0800
\par 修改时间:
	2023-05-21 11:29 +0800
\par 文本编码:
	UTF-8
\par 模块名称:
	NPL::NPLA1Root
...
#include YFM_NPL_NPLA1Root // for ystdex::value_or, TryAccessReferencedTerm,
//	IsBranch, ThrowInsufficientTermsError, NPL::Deref, ReduceSubsequent,
//	A1::NameTypedReducerHandler, ReduceOnceLifted, std::next,
//	ReduceReturnUnspecified, AccessFirstSubterm, RemoveHead, ReduceOrdered,
//	LiftOtherValue, RetainList, std::bind, std::ref, std::placeholders,
//	TNCIter, IsSticky, ReferenceTerm, RelaySwitched, trivial_swap,
//	BindingMap, shared_ptr, Environment, std::throw_with_nested, TypeError,
//	ContextHandler, YSLib::value_map, string, TokenValue, string_view,
//	ValueObject, any_ops::use_holder, in_place_type, YSLib::HolderFromPointer,
//	NPL::allocate_shared, InvalidReference, BindParameter, Retain,
//	ClearCombiningTags, MoveFirstSubterm, std::declval, ResolveEnvironment,
//	BindParameterWellFormed, EnvironmentReference, ystdex::sfmt,
//	TermToStringWithReferenceMark, ResolveTerm, ResolvedTermReferencePtr,
//	LiftOtherOrCopy, NPL::IsMovable, RetainN, NPL::ResolveRegular,
//	EnvironmentParent, NPL::ToParent, SingleWeakParent, SingleStrongParent,
//	Environment::ThrowForInvalidType, TNIter, ystdex::make_transform,
//	TryAccessLeafAtom, TermReference, ParentList, ystdex::as_const,
//	NPL::AllocateEnvironment, IsTypedRegular, TryAccessLeaf, IsList,
//	ystdex::fast_all_of, IsEmpty, AssertValueTags, BindSymbol,
//	ThrowListTypeErrorForNonList, TermTags, LiftOther, NPL::AssignParent,
//	ystdex::equality_comparable, YSLib::Debug, YSLib::sfmt,
//	GuardFreshEnvironment, NPLException, CheckParameterTree, std::allocator_arg,
//	NPL::TransferSubtermsBefore, ystdex::expand_proxy, AccessRegular,
//	GetLValueTagsOf, NPL::AsTermNode, RegularizeTerm, IsPair,
//	LiftPropagatedReference, LiftTermOrCopy, LiftMovedOther, EnsureValueTags,
//	IsLeaf, LiftTerm, ThrowValueCategoryError, IsAtom,
//	ThrowListTypeErrorForAtom, NPL::TransferSubtermsAfter,
//	ThrowInvalidSyntaxError, ShareMoveTerm, ystdex::exchange,
//	ExtractEnvironmentFormal, A1::MakeForm, RetainVariadicList, type_id,
//	HasStickySubterm, LiftToReturn, ystdex::prefix_eraser, IsTyped,
//	IsBranchedList, EnvironmentGuard, NPL::MoveParentValue
//	NPL::AssignWeakParent, NPL::AssignParentH, lref, NPL::TransferSubterms,
//	A1::AsForm, ystdex::bind1, ystdex::ref_eq, ystdex::equal_to, LiftCollapsed;
#include YFM_NPL_NPLA1Forms // for ExtractBool, RetainListArguments,
//	MakeValueOrMove, ReduceMakeForm, Forms::CallResolvedUnary,
//	Forms::CallRegularUnaryAs, Forms::CallUnary, ReduceForwardHandler;
...
ReduceCond(TermNode& term, ContextNode& ctx, TNIter i)
{
	if(i != term.end())
	{
		...
		return ReduceSubsequent(NPL::Deref(j), ctx,
			...
			return CondTest(clause, j) ? ReduceOnceLifted(term, c, clause)
				: ReduceCond(term, c, std::next(i));
		}, "eval-cond-list"));
	}
...
{
	auto& test(*RetainListArguments(term));
		...
		MakeParameterValueMatcher(a, [&](const TokenValue& n){
			...
			if(store.find(k) == store.cend())
				...
				Environment::Bind(m, k, TermNode(TermNode::Container(a),
					...
					store[k] = NPL::allocate_shared<ContextHandler>(a,
...
//! \since build 974
ReductionStatus
EvalImplUnchecked(TermNode& term, ContextNode& ctx, bool no_lift, bool at = {})
{
...
//! \since build 974
ReductionStatus
EvalImpl(TermNode& term, ContextNode& ctx, bool no_lift, bool at = {})
{
	RetainN(term, 2);
...
{
	// XXX: See %MakeEnvironmentParentList.
	if(const auto p = vo.AccessPtr<EnvironmentReference>())
		return NPL::ToParent<SingleWeakParent>(a, *p);
	if(const auto p = vo.AccessPtr<shared_ptr<Environment>>())
		return NPL::ToParent<SingleStrongParent>(a, *p);
	// NOTE: This does not accept parent environment values other than
	//	first-class objects.
...
ToParentListElement(TermNode::allocator_type a, ValueObject&& vo)
{
	// XXX: See %MakeEnvironmentParentList.
	if(const auto p = vo.AccessPtr<EnvironmentReference>())
		return NPL::ToParent<SingleWeakParent>(a, std::move(*p));
	if(const auto p = vo.AccessPtr<shared_ptr<Environment>>())
		return NPL::ToParent<SingleStrongParent>(a, std::move(*p));
	// NOTE: Ditto.
	Environment::ThrowForInvalidType(vo.type());
}
	...
	bool move)
{
	...
	const auto tr([&](TNIter iter){
		return ystdex::make_transform(iter, [&](TNIter i) -> EnvironmentParent{
			// XXX: Like %LiftToReturn, but for %Value only, and finally
			//	filtered by %MakeParentListElement.
	...
	pair<shared_ptr<Environment>, bool>&& pr)
{
	...
	if(pr.second)
		return NPL::ToParent<SingleStrongParent>(a, std::move(p_env));
...
CreateEnvironment(TermNode& term)
{
	return NPL::AllocateEnvironment(term.get_allocator(),
		MakeResolvedParentList(term, true));
}
...
//! \since build 964
void
VauPrepareCall(ContextNode& ctx, TermNode& term, EnvironmentParent& parent,
...
public:
	...
	//	constructor can be defined, but unnecessary as target object of
	//	%ContextHandler. The assignment operators are not used, and they would
	//	be deleted if defined as defaulted (due to the type of %call).
...
{
	NPL::TransferSubtermsBefore(t, nd, nd.begin());
}
	...
	ResolvedTermReferencePtr p_ref)
{
	if(!fch.IsOperative())
		return MakeValueOrMove(p_ref, [&]{
			...
			fch.Unwrap();
			return ReduceMakeForm(term, std::move(fch));
		});
	ThrowUnwrappingFailureOnOperative();
}
...
BranchRestFwdReferenced(TermNode& term, TermNode& nd, bool move)
{
	...
	if(move)
	{
		...
		NPL::TransferSubtermsAfter(tcon, nd, first, last);
...
MakeResolvedParent(TermNode& nd, ResolvedTermReferencePtr p_ref)
{
	...
	// NOTE: Currently all supported environment representations of a single
	//	object are in the leaf, or in the list with 1 elements which optimize as
	//	the case of leaf.
	if(IsList(nd))
		return MakeResolvedParentList(nd, move);
	if(IsLeaf(nd))
		return MakeResolvedParentLeaf(nd, p_ref);
	ThrowInvalidEnvironmentType(nd, p_ref);
}

//! \since build 964
YB_ATTR_nodiscard YB_FLATTEN EnvironmentParent
...
struct GReduceVau final
{
	...
	operator()(TermNode& term, bool no_lift, TNIter i, EnvironmentParent&& ep,
		_tParams&&... args) const
	{
		return ReduceCreateFunction(term, [&]() -> ContextHandler{
			...
			if(eformal)
				return A1::MakeForm(term, DynamicVauHandler(std::move(formals),
					std::move(ep), std::move(p_es), no_lift,
...
struct GReduceVau<true> final
{
		...
		_tParams&&... args) const
	{
		return ReduceCreateFunction(term, [&]{
			...
			return A1::MakeForm(term, VauHandler(std::move(formals),
				std::move(ep), MakeCombinerEvalStruct(term, ++i), no_lift),
				yforward(args)...);
		});
	}
};
...
LambdaVau(TermNode& term, ContextNode& ctx, bool no_lift)
{
	...
	RetainVariadicList(term, _vN);
	return GReduceVau<_vN == 0>()(term, no_lift, term.begin(),
		NPL::ToParent<SingleWeakParent>(term.get_allocator(),
	...
	FormContextHandler& fch, size_t n)
{
	...
	// XXX: Ditto.
	return ReduceMakeForm(term, std::move(fch.Handler), n);
}
...
template<ReductionStatus(&_rWrap)(TermNode&, ResolvedTermReferencePtr,
	FormContextHandler&, size_t), typename _func>
ReductionStatus
...
ApplyTail(TermNode& term, ContextNode& ctx, TNIter i)
{
	...
	auto p_env([&]() -> shared_ptr<Environment>{
		if(term.size() == 2)
			return NPL::AllocateEnvironment(term.get_allocator());
		if(term.size() == 3)
			return FetchValidEnvironment(ResolveEnvironment(*++i).first);
...
ListAsteriskTail(TermNode& term)
{
	...
	--last;
	NPL::TransferSubtermsAfter(term, *last),
	term.Value = std::move(last->Value);
	term.Remove(last);
	return ReductionStatus::Retained;
}
//!@}
	...
	ReductionStatus(&f)(TermNode&, ContextNode&))
{
	...
	rterm.Clear();
	NPL::TransferSubtermsAfter(rterm, term, ++i, term.end());
...
PrependList(TermNode::Container& tcon, TermNode& tm)
{
	...
	NPL::ResolveTerm([&](TermNode& nd, ResolvedTermReferencePtr p_ref){
		if(IsList(nd))
		{
			...
			ThrowListTypeErrorForNonList(nd, true);
		NPL::TransferSubtermsBefore(tcon, tm);
	}, tm);
}
...
LetCombineBody(_func f, TermNode& term, ContextNode& ctx)
{
	...
	//	as in %VauBind.
	NPL::MoveParentValue(ctx, operand.Value);
...
LetCombineEmpty(TermNode& term, ContextNode& ctx, bool no_lift)
{
	...
	NPL::MoveParentValue(ctx, i->Value);
...
LetEmptyGuardTail(_func f, TermNode& term, ContextNode& ctx)
{
	...
	NPL::AssignParentH<SingleWeakParent>(ctx, term, std::move(r_env));
	return f(gd);
}
...
LetEmpty(TermNode& term, ContextNode& ctx, bool no_lift, bool with_env)
{
	...
	return LetCombineNextParent([&]{
		...
		NPL::MoveParentValue(ctx, term.begin()->Value);
		return LetCallTail(term, ctx, gd, no_lift);
	}, term, ctx, ++i, parent);
#else
...
#	endif
	...
	else if(IsBranchedList(extracted))
	{
		...
		NPL::TransferSubtermsAfter(*con.emplace_front().emplace(), extracted,
...
{
	RetainVariadicList(term, with_env ? 1 : 0);
...
{
	NPL::TransferSubtermsAfter(bindings, term, i, term.end());
...
ProvideLetCommon(TermNode& term, ContextNode& ctx)
{
	...
	if(IsList(nd))
		...
		ThrowListTypeErrorForNonList(nd, true);
	NPL::TransferSubterms(con, ++++++i, con, i_symbols);
	...
	con.emplace(i_symbols, A1::AsForm(a, ystdex::bind1(
		...
		body.Clear();
		NPL::TransferSubtermsAfter(tcon, con, i, con.end());
...
{
	const auto i(RetainListArguments(term));
...
RestVal(TermNode& term)
{
	return RestOrVal(term, [&] YB_LAMBDA_ANNOTATE((TermNode::Container& tcon,
		...
		if(NPL::IsMovable(p_ref))
		{
			...
			//	subterm in %nd.
			NPL::TransferSubtermsAfter(tcon, con, first, last);
			...
			NPL::TransferSubtermsAfter(tcon, con, con.begin(),
				LiftPrefixToReturn(nd));
			LiftToReturn(nd);
			NPL::TransferSubtermsBefore(con, tcon, std::next(tcon.begin()),
				tcon.end());
			term.Value = std::move(nd.Value);
		}
		else
		{
			...
			if(const auto p = TryAccessValue<const TermReference>(nd.Value))
			{
				auto& src(p->get());
#if false
				// XXX: This seems no more inefficient.
				TermNode::Container ncon(src.get_allocator());

				CopyRestContainer(ncon, src.begin(), src.end());
				NPL::TransferSubtermsAfter(tcon, ncon);
#else
				NPL::TransferSubtermsAfter(tcon, TermNode::Container(
					src.GetContainer(), src.get_allocator()));
#endif
...
ReductionStatus
EvalAt(TermNode& term, ContextNode& ctx)
{
...
Wrap(TermNode& term)
{
	return WrapOrRef<WrapN>(term,
		[&](ContextHandler& h, ResolvedTermReferencePtr p_ref){
		return ReduceForwardHandler(term, h, p_ref);
	});
}
...
WrapRef(TermNode& term)
{
	return WrapOrRef<WrapRefN>(term,
		[&](ContextHandler& h, ResolvedTermReferencePtr p_ref){
		// XXX: Allocators are not used, as %ReduceForwardHandler.
		return p_ref ? ReduceForCombinerRef(term, *p_ref, h, 1)
			: ReduceMakeForm(term, std::move(std::move(h)), 1);
	});
}
	...
	CheckFunctionCreation([&]{
		ResolveTerm([&](TermNode& nd, ResolvedTermReferencePtr p_ref){
			if(!IsExtendedList(nd))
				...
				yunused(MakeEnvironmentParentList(nd.begin(), nd.end(),
					nd.get_allocator(), true));
			else
				ThrowInvalidEnvironmentType(nd, p_ref);
		}, tm);
	});
...
MakeEncapsulationType(TermNode& term)
{
	...
	//	shared_ptr<void> p_type(NPL::allocate_shared<yimpl(byte)>(a));
...
{
	auto i(RetainListArguments(term));
...
Append(TermNode& term)
{
...

	{
		...
		tm.Clear();
		NPL::TransferSubtermsAfter(tm, term, ++i, term.end());
	}
#endif
	...
	ResolveTerm([&](TermNode& nd, ResolvedTermReferencePtr p_ref){
		if(!p_ref || p_ref->IsModifiable())
		{
			if(IsList(nd))
			{
				...
				NPL::TransferSubtermsBefore(nd, term, i);
			}
			else
				ThrowListTypeErrorForNonList(nd, p_ref);
		}
	...
	if(IsList(eterm))
		...
		return A1::RelayCurrentNext(ctx, eterm, trivial_swap,
			...
			if(con.size() > 2)
			{
				...
				return DoBindingsToEnvironment(term, ctx, p_env, ++++j, eterm,
					operand);
			}
			con.pop_front();
