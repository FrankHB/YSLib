...
//	Replaced all "NULL" with "nullptr", to avoid possible G++ warning [-Wzero-as-null-pointer-constant]. This requires the "nullptr" keyword being supported.
//	Removed static function "jpeg_read_icc_profile" to avoid it clashing with public function declared in "jepglib.h".
//	Removed unused static function "marker_is_icc". Previously it is only referenced by "jpeg_read_icc_profile".
...
Save(FreeImageIO *io, FIBITMAP *dib, fi_handle handle, int page, int flags, void *data) {
	if ((dib) && (handle)) {
		try {
			...
			if(FreeImage_GetThumbnail(dib) != nullptr) {
