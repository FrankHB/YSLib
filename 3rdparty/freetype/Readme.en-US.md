...
Tested with `CFLAGS ?## -c -g -O3 -Wall` in the makefile. Option `-g` is removed to reduce binary image size.
...
This requires more `CPPFLAGS`. See the instructions for MinGW32 build to work around.
