...
#else
	return Path(FetchCurrentWorkingDirectory<char16_t>(IO::MaxPathLength));
#endif
}
	...
	btnSwitchMain(*this, {234, 170}), btnSwitchSub(*this, {234, 170})
{
	...
	tpDefault.BindByFocus(tbTest, &TextPlaceholder::SwapTextBox<TextBox>),
...
//	btnTestEx.Enabled = {},
	...
	fbMain.SetPath(pth),
	dsk_s.BoundControlPtr = [&](const KeyInput& k) -> observer_ptr<IWidget>{
		if(k.count() == 1)
		{
			if(k[YCL_KEY(A)])
