...
public:
	...
	//检查文件头是否有 BOM(Byte Order Mark) ，若有则据此判断编码，返回 BOM 长度。
	u8
	CheckBOM(Text::CSID&);
	...
	//设置文件读位置，whence 语义同 fseek 函数（SEEK_SET 起始为文本区段头）。
	void
	...
	//从文件读 n 字节（按默认编码转化为 UTF-16LE）到 s 中。
	SizeType
